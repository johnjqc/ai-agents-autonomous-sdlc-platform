...
2. If the branch is still `main`, create a new branch with name `feature/<issueId>-<summary-in-kebab-case-max-5-words>` and Run `git checkout -b {bracnh_name}`.
3. Run `git add .` to stage all changes.
4. Run `git commit -m "feat({ticket_id}): {ticket_summary}"` or the git spects skill for commit.
5. Run `git push origin {branch_name}` to push the current branch to the remote.
6. Call `mcp__github__create_pull_request` with `draft: true` only after
   Step 4 succeeds.
7. Return the PR URL to the orchestrator.
